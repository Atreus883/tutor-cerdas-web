...
// Anda bisa memindahkan 'styles' ke file CSS terpisah
const styles = `
...
.ad-card{ background:var(--panel); border:1px solid var(--line); border-radius:16px; padding:16px; box-shadow:0 10px 24px rgba(0,0,0,.08); }
...
.ad-input{ width:100%; padding:10px 12px; border-radius:10px; border:1px solid var(--line); background:var(--soft); color:inherit; outline:none; }
.ad-btn{ border:1px solid var(--line); background:linear-gradient(180deg, rgba(255,255,255,.08), rgba(255,255,255,.02)); color:inherit; padding:10px 14px; border-radius:12px; font-weight:700; cursor:pointer; transition:transform .12s ease; }
...
.ad-badge.processing{ background:rgba(245,158,11,.12); color:#d97706; border-color:rgba(217,119,6,.25);}
.ad-badge.err{ background:rgba(239,68,68,.12); color:#ef4444; border-color:rgba(239,68,68,.25);}
.ad-drop{ margin-top:8px; padding:18px; border:1.5px dashed var(--line); border-radius:12px; background:var(--soft); text-align:center; color:var(--muted); }
...
export default function Admin() {
  // ==========================================================
  // PERUBAHAN UTAMA: Gunakan fetchWithAuth dan isAuthenticated dari context
  // ==========================================================
  ...
  const [loading, setLoading] = useState(false); // Loading untuk data list
  ...
  // PERUBAHAN: Gunakan fetchWithAuth untuk semua panggilan
  async function refresh() {
    try {
      setLoading(true);
      setErrorMsg("");
      const j = await fetchWithAuth(`/documents`);
      setItems(j.items || []);
    } catch (e) {
  ...
  // PERUBAHAN: useEffect sekarang bergantung pada isAuthenticated
  useEffect(() => {
    // Hanya jalankan refresh jika pengguna sudah terautentikasi
    if (isAuthenticated) {
      refresh();
    }
  }, [isAuthenticated]); // <-- Ini akan memicu refresh saat login selesai
  ...
  async function upload() {
    ...
    try {
      ...
      // Gunakan fetchWithAuth untuk upload
      await fetchWithAuth(`/documents/upload`, {
        method: "POST",
        body: fd,
        // headers tidak perlu di-set, fetch akan otomatis mengaturnya untuk FormData
      });
  ...
  async function rebuild(id) {
    try {
      setRebuildId(id);
      setErrorMsg("");
      // Gunakan fetchWithAuth untuk rebuild
      const j = await fetchWithAuth(`/documents/rebuild/${id}`, {
        method: "POST",
      });
      if (j.ok) {
  ...
  async function viewChunks(id) {
    try {
      setViewDoc(id);
      setChunks([]);
      // Gunakan fetchWithAuth untuk melihat chunks
      const j = await fetchWithAuth(`/documents/${id}/chunks?limit=200`);
      setChunks(j.items || []);
    } catch (e) {
      setErrorMsg(String(e.message || e));
  ...
  // Sisa kode (UI) tidak perlu diubah secara signifikan
  // ... (Kode JSX Anda seperti statusBadge, filtered, onDrop, dan return() tetap sama) ...
  function statusBadge(status) {
    const s = (status || "").toLowerCase();
    if (s === "embedded") return <span className="ad-badge ok">embedded</span>;
    if (s === "processing")
      return <span className="ad-badge processing">processing</span>;
                  ...
                  <li key={x.id} className="ad-item">
                    ...
                    >
                      <div style={{ minWidth: 0 }}>
                        <div className="ad-title">{x.title || x.id}</div>
